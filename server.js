...
`);
    // pre_conversiones_empaque eliminado: ahora el modelo usa pre_codigos_articulo + pre_factores_empaque

db.run(`
    CREATE TABLE IF NOT EXISTS pre_codigos_articulo (
        id INTEGER PRIMARY KEY AUTOINCREMENT,

        articulo TEXT NOT NULL,
        codigo TEXT NOT NULL,
        tipo_codigo TEXT NOT NULL,     -- 'interno' o 'externo'
...
db.run(`
    CREATE TABLE IF NOT EXISTS pre_factores_empaque (
        id INTEGER PRIMARY KEY AUTOINCREMENT,

        articulo TEXT NOT NULL,
        modo_escaneo TEXT NOT NULL,      -- 'pallet', 'caja_pieza', 'caja_master'

        factor_conversion REAL DEFAULT 1,
        unidad_base TEXT,                -- PIEZA, CAJA, etc.
        descripcion TEXT,
...
`);
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_pre_codigos_articulo_articulo ON pre_codigos_articulo(articulo)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_pre_codigos_articulo_codigo ON pre_codigos_articulo(codigo)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_pre_factores_empaque_articulo ON pre_factores_empaque(articulo)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_pre_factores_empaque_modo ON pre_factores_empaque(modo_escaneo)`);
...
// ======================================================
app.post('/api/modificar-turno', (req, res) => {
    const {
        id,
        folio,
        nombre_operador,
    ...
    db.get(`SELECT * FROM Turnos WHERE id = ?`, [id], (findErr, turnoActual) => {
        if (findErr) {
            console.error('Error al consultar turno actual:', findErr.message);
        ...
        const estatusFinal = estatus || turnoActual.estatus || 'En Espera';
        const veniaDeProgramado = String(turnoActual.estatus || '').trim() === 'Programado';
        const sigueProgramado = String(estatusFinal || '').trim() === 'Programado';

        // Si estaba programado y ahora ya no lo está, marcar hora real de llegada
        const debeMarcarLlegadaReal = veniaDeProgramado && !sigueProgramado;
        ...
        const query = `
            UPDATE Turnos SET
                ...
                tipo_operacion = ?,
                hora_llegada = ${
                    debeMarcarLlegadaReal
                        ? "datetime('now', 'localtime')"
                        : "COALESCE(hora_llegada, datetime('now', 'localtime'))"
                }
            WHERE id = ?
        `;
...
// Endponit: Para escaneo de conversion //

app.post('/api/turnos/:turnoId/pre-documentos/:documentoId/escanear', (req, res) => {
    const turnoId = Number(req.params.turnoId);
    const documentoId = Number(req.params.documentoId);
    const codigo = String(req.body.codigo || '').trim().toUpperCase();
    const modoEscaneo = String(req.body.modo_escaneo || '').trim().toLowerCase();
    const usuario = req.session?.user?.nombre || 'Sistema';

    if (!turnoId || !documentoId || !codigo || !modoEscaneo) {
        return res.status(400).json({ error: 'Faltan datos para escanear.' });
    }

    const modosValidos = ['pallet', 'caja_pieza', 'caja_master'];
    if (!modosValidos.includes(modoEscaneo)) {
        return res.status(400).json({ error: 'Modo de escaneo inválido.' });
    }
    ...
    db.get(
        ...
        (err, vinculo) => {
            ...
            if (!vinculo) {
                return res.status(404).json({ error: 'Documento no ligado a esta unidad.' });
            }

            if (!['en_validacion', 'validado', 'con_incidencias'].includes(vinculo.estado_validacion)) {
                return res.status(409).json({ error: 'Debes iniciar validación antes de escanear.' });
            }

            // 1) Resolver artículo por código interno/externo
            db.get(
                `
                SELECT articulo, tipo_codigo
                FROM pre_codigos_articulo
                WHERE UPPER(codigo) = ?
                  AND activo = 1
                LIMIT 1
                `,
                [codigo],
                (codErr, codigoArticulo) => {
                    if (codErr) {
                        console.error('Error al buscar código de artículo:', codErr.message);
                        return res.status(500).json({ error: codErr.message });
                    }

                    if (!codigoArticulo) {
                        return res.status(404).json({ error: 'Código no encontrado en catálogo de artículos.' });
                    }

                    // 2) Obtener factor según modo elegido
                    db.get(
                        `
                        SELECT *
                        FROM pre_factores_empaque
                        WHERE UPPER(articulo) = ?
                          AND modo_escaneo = ?
                          AND activo = 1
                        LIMIT 1
                        `,
                        [String(codigoArticulo.articulo).trim().toUpperCase(), modoEscaneo],
                        (facErr, factorRow) => {
                            if (facErr) {
                                console.error('Error al buscar factor de empaque:', facErr.message);
                                return res.status(500).json({ error: facErr.message });
                            }

                            if (!factorRow) {
                                return res.status(404).json({
                                    error: `No existe factor de conversión para el artículo ${codigoArticulo.articulo} en modo ${modoEscaneo}.`
                                });
                            }

                            // 3) Buscar línea operativa del PRE actual
                            db.get(
                                `
                                SELECT tpp.*
                                FROM turno_pre_partidas tpp
                                INNER JOIN pre_partidas pp ON pp.id = tpp.pre_partida_id
                                WHERE tpp.turno_pre_documento_id = ?
                                  AND UPPER(pp.articulo) = ?
                                LIMIT 1
                                `,
                                [vinculo.id, String(codigoArticulo.articulo).trim().toUpperCase()],
                                (lineErr, linea) => {
                                    if (lineErr) {
                                        console.error('Error al buscar línea PRE por artículo:', lineErr.message);
                                        return res.status(500).json({ error: lineErr.message });
                                    }

                                    if (!linea) {
                                        return res.status(404).json({
                                            error: `El artículo ${codigoArticulo.articulo} no existe en este PRE.`
                                        });
                                    }

                                    const qty = Number(factorRow.factor_conversion || 1);
                                    const nuevaCantidad = Number(linea.cantidad_recibida || 0) + qty;
                                    const diferencia = nuevaCantidad - Number(linea.cantidad_esperada || 0);
                                    const estadoLinea = calcularEstadoLinea(
                                        linea.cantidad_esperada,
                                        nuevaCantidad,
                                        linea.incidencia_tipo
                                    );
                                    ...
                                    db.run(
                                        ...
                                        function (lectErr) {
                                            if (lectErr) {
                                                console.error('Error al registrar lectura:', lectErr.message);
                                                return res.status(500).json({ error: lectErr.message });
                                            }
                                            ...
                                            db.run(
                                                ...
                                                WHERE id = ?
                                                `,
                                                [nuevaCantidad, diferencia, estadoLinea, usuario, linea.id],
                                                function (updErr) {
                                                    if (updErr) {
                                                        console.error('Error al actualizar línea PRE:', updErr.message);
                                                    ...
                                                    recalcularResumenTurnoPre(vinculo.id, (resumenErr, resumen) => {
                                                        ...
                                                        return res.json({
                                                            ok: true,
                                                            articulo: codigoArticulo.articulo,
                                                            tipo_codigo: codigoArticulo.tipo_codigo,
                                                            modo_escaneo: modoEscaneo,
                                                            factor_conversion: qty,
                                                            linea_id: linea.id,
                                                            cantidad_recibida: nuevaCantidad,
                                                            diferencia,
                                                            estado_linea: estadoLinea,
